...
def generar_tiempo_aleatorio(descripcion):
    ...
    patron_esp = re.compile(
        r"(?:un tiempo aleatorio )?entre (\d+|[\w]+) y (\d+|[\w]+) horas"
    )
    patron_ing = re.compile(
        r"(?:a random time )?between (\d+|[\w]+) and (\d+|[\w]+) hours"
    )
    ...
    if match_esp:
        ...
        raise ValueError(
            f"No se reconoce el formato de tiempo aleatorio: {descripcion}"
        )
    ...
    for valor, unidad in componentes:
        ...
        if es_ingles:
            ...
            if "hora" in unidad:
                ...
                segundos = numero
    print(
        f"Valores procesados: horas={horas}, minutos={minutos}," " segundos={segundos}"
    )
...
def step_when_wait_time_description(context, time_description):
    ...
    # Verificar si es un número directo seguido de horas/hour
    elif re.match(r"^(\d+\.?\d*)\s*horas?$", time_description) or re.match(
        r"^(\d+\.?\d*)\s*hours?$", time_description
    ):
        ...
        if match:
            ...
            print(
                f"Grupos capturados: horas='{hours_word}',"
                " minutos='{minutes_word}', segundos='{seconds_word}'"
            )
            ...
            print(
                f"Valores convertidos: horas={hours},"
            ...
            total_time_in_hours = procesar_descripcion_tiempo(
                time_description, es_ingles
            )
...
def step_then_should_raise_negative_error(context):
    ...
    ), "Se esperaba una excepción, pero no ocurrió ninguna."
    assert isinstance(context.exception, CantidadNegativaError), (
        f"Se esperaba CantidadNegativaError, pero se obtuvo:"
...
def step_then_should_raise_excessive_error(context):
    ...
    assert isinstance(context.exception, CantidadExcesivaError), (
        f"Se esperaba CantidadExcesivaError, pero se obtuvo:"
        " {type(context.exception)}"
    )
...
def step_then_can_eat_more(context, cantidad):
    ...
    assert context.pepinos_restantes == cantidad, (
        f"Se esperaba poder comer {cantidad} pepinos más, pero se puede"
        " comer {context.pepinos_restantes}"
    )
...
def step_then_will_growl_in(context, horas):
    ...
    assert (
        context.tiempo_restante is not None
    ), "El estómago no gruñirá con esta cantidad de pepinos"
    assert abs(context.tiempo_restante - horas) < 0.01, (
        f"Se esperaba que gruñera en {horas} horas, pero gruñirá en"
        " {context.tiempo_restante} horas"
    )
...
def step_then_confirm_will_growl(context):
    ...
    assert context.prediccion_gruñido is True, (
        "Se esperaba que el estómago gruñera, pero la predicción dice" " que no gruñirá"
    )
...
def step_then_confirm_will_not_growl(context):
    ...
    assert context.prediccion_gruñido is False, (
        "Se esperaba que el estómago no gruñera, pero la predicción"
        " dice que sí gruñirá"
    )
...
def step_then_eaten_cukes(context, cantidad):
    ...
    assert context.total_pepinos == cantidad, (
        f"Se esperaba haber comido {cantidad} pepinos, pero se comieron"
        " {context.total_pepinos}"
    )
...
def step_then_available_cukes(context, cantidad):
    ...
    assert context.pepinos_disponibles == cantidad, (
        f"Se esperaba poder comer {cantidad} pepinos más, pero se"
        " pueden comer {context.pepinos_disponibles}"
    )
...
def step_then_digestion_time(context, horas):
    ...
    assert context.tiempo_digestion == horas, (
        f"Se esperaba un tiempo de digestión de {horas} horas, pero se"
        " obtuvo {context.tiempo_digestion} horas"
    )
